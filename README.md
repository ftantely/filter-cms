...
<p align="center">
  ...
  <p align="center">
    Introducing the official intermediate starter kit, presented by CodeStitch. This kit includes a pre-configured Eleventy setup that utilizes Nunjucks templating, along with a seamless integration of Decap CMS, providing an easy way to manage a cms. Everything is ready to go right from the start, offering a fantastic cs-content to the advantages of a Static Site Generator, complete with LESS preprocessing.
    <br/>
    <br/>
...
allows for repeated components, centralized data and greater room to scale as your clients grow. On top of this, a cms has been provided through
...
3. A more applied article about leveraging Nunjucks/Eleventy to make your code modular can be [found here](https://www.webstoemp.com/cms/modular-code-nunjucks-eleventy/), courtesy of Webstoemp.
...
-   content/ - Pages or data to render pages from, such as the cms.
...
> In this case, if the page slug is "about", the .cs-active class will be applied. You're welcome to adjust the page slug value to whatever you require ("cms", "/", "services", etc)
...
Within the `src/` directory, you'll find an `admin/` folder which contains the configuration for the cms, alongside an entry `index.html` file, which you
shouldn't need to worry about. While this project is set up to work with a cms out of the box, you are welcome to make changes to the `config.yaml` file using
<a href="https://decapcms.org/docs/add-to-your-site/#configuration">Decap CMS'</a> documentation.

cms content lives in `/src/content/cms` in the form of markdown files, with a front matter similar to that of the pages. The cms post layout, tags and
permalinks are defined in the `cms.json` file in the same directory, while all cms-related media lives in `src/assets/images/cms`.
...
taken, so look out for that. You can locally access the cms via navigating to the /admin path. All cms content can be easily created, updated and deleted via
this admin panel, and is the very system that your clients can use to manage their website without your involvement. Everything on the cms should be fairly
